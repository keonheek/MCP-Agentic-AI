...
# Walk up to 6 levels to find .env (workspace root may be deep)
_here = Path(__file__).resolve()
for _p in [_here.parents[i] / '.env' for i in range(6)]:
    if _p.exists():
...
def _get_headers() -> dict:
    key = os.getenv("PERPLEXITY_API_KEY") or PERPLEXITY_API_KEY
    if not key:
...
HEADERS: dict = {}  # populated lazily via _get_headers()
...
def _perplexity_query(prompt: str) -> str:
    ...
    try:
        resp = requests.post(PERPLEXITY_URL, headers=_get_headers(), json=payload, timeout=REQUEST_TIMEOUT)
...
def _find_website_url(corp_name: str) -> str | None:
    ...
    # Strip markdown bold/italic markers before URL extraction
    response_clean = re.sub(r'\*{1,2}', '', response)
    skip_domains = {'naver', 'daum', 'google', 'kakao', 'instagram', 'facebook', 'linkedin', 'youtube', 'wikipedia', 'namuwiki'}

    # 1. Extract full URLs (https://...)
    urls = re.findall(r'https?://[^\s\)\]\,\"\']+', response_clean)

    # 2. Extract bare domains with strict known TLDs (avoids false positives)
    _known_tlds = r'(?:com|co\.kr|kr|net|org|io|ai|app|shop|store|biz|info)'
    bare = re.findall(rf'(?:www\.)?[a-zA-Z0-9\-]+\.{_known_tlds}(?:/[^\s\)\]\,\"\']*)?', response_clean)
    urls += [f"https://{u}" for u in bare if not u.startswith('http')]
